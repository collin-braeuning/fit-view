...
| II. Self-Contained, Reusable Components | ✅ Pass | `SessionCard` owns its expanded/collapsed state as `@State private var isExpanded`; the parent (`BatchOverviewCardList`) no longer lifts a `Set<String>` on its behalf. An earlier revision of this plan recorded the lifted state as an accepted deviation on the grounds that per-card `@State` would reset on a `model.rows` reload — that claim was tested and found false, and the state was moved into the card. See research.md §2. |
